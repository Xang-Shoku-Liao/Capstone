...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  static const _detectorChannel = MethodChannel(
    'com.capstone.finanzas_app/detector',
  );
  ...
  void initState() {
    ...
    _detectorChannel.setMethodCallHandler((call) async {
      if (call.method == 'transaccionAutomaticaRegistrada') {
        await _cargarDatos();
      }
    });
    _cargarDatos();
    _cargarEstadoDetector();
  }

  @override
  void dispose() {
    WidgetsBinding.instance.removeObserver(this);
    _detectorChannel.setMethodCallHandler(null);
    super.dispose();
  }
  ...
  Future<void> _cargarEstadoDetector() async {
    if (!Platform.isAndroid) return;
    try {
      final activo =
          await _detectorChannel.invokeMethod<bool>('detectorActivo') ?? false;
      if (mounted) setState(() => _detectorActivo = activo);
    } on PlatformException {
      // El detector solo existe en la versión Android de la aplicación.
    } on MissingPluginException {
      // Permite ejecutar la pantalla en otras plataformas durante desarrollo.
    }
  }

  Future<void> _activarDetector() async {
    try {
      await _detectorChannel.invokeMethod<void>('abrirAjustesDetector');
    } on PlatformException {
      if (mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Disponible solo en Android.')),
        );
      }
    }
  }
              ...
              child: CustomScrollView(
                slivers: [
                  ...
                  if (Platform.isAndroid)
                    SliverToBoxAdapter(child: _buildDeteccionAutomatica()),
  ...
  Widget _buildDeteccionAutomatica() {
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            CircleAvatar(
              backgroundColor: _detectorActivo
                  ? const Color(0xFFE0F4EF)
                  : const Color(0xFFF1EFE8),
              foregroundColor: _detectorActivo ? const Color(0xFF1D9E75) : teal,
              child: Icon(
                _detectorActivo
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    _detectorActivo
                        ? 'Activa para notificaciones de Banco Demo.'
                        : 'Actívala para importar transacciones de prueba.',
            ...
            TextButton(
              onPressed: _detectorActivo ? null : _activarDetector,
              child: Text(_detectorActivo ? 'Activa' : 'Activar'),
            ),
          ],
        ),
      ),
